<template>
  <div class="post" v-for="(postData,idx) in postData" :key="idx">
    <div class="post-header">
      <div class="profile"></div>
      <span class="profile-name">{{postData.name}}</span>
    </div>
    <div :class="`post-body ${postData.filter}`" :style="{ backgroundImage : `url(${postData.postImage})` }" @click="$store.commit('사진좋아요')"></div>
